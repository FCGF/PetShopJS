...
namespace PetShopJS.Controllers {
    public class Produto_EmpresaController : Controller {
        ...
        public JsonResult Create([Bind(Include = "Id,IdEmpresa,IdProduto,Valor,Estoque,Ativo")] Produto_Empresa produto_Empresa) {
            if (ModelState.IsValid) {
                db.Produto_Empresa.Add(produto_Empresa);
                db.SaveChanges();
                return Json(new { result = true, message = "Produto - Empresa criada com sucesso" });
            } else {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult Edit([Bind(Include = "Id,IdEmpresa,IdProduto,Valor,Estoque,Ativo")] Produto_Empresa produto_Empresa) {
            if (ModelState.IsValid) {
                db.Entry(produto_Empresa).State = EntityState.Modified;
                db.SaveChanges();
                return Json(new { result = true, message = "Produto - Empresa editada com sucesso" });
            } else {
                IEnumerable<ModelError> errors = ModelState.Values.SelectMany(i => i.Errors);
        ...
        public JsonResult DeleteConfirmed(int id) {
            ...
            db.SaveChanges();
            return Json(new { result = true, message = "Produto - Empresa deletada com sucesso" });
        }
