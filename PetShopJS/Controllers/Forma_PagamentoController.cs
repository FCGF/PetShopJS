...
namespace PetShopJS.Controllers {
    public class Forma_PagamentoController : Controller {
        ...
        public JsonResult Create([Bind(Include = "Id,Nome")] Forma_Pagamento forma_Pagamento) {
            if (ModelState.IsValid) {
                db.Forma_Pagamento.Add(forma_Pagamento);
                db.SaveChanges();
                return Json(new { result = true, message = "Forma de Pagamento criada com sucesso" });
            } else {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult Edit([Bind(Include = "Id,Nome")] Forma_Pagamento forma_Pagamento) {
            if (ModelState.IsValid) {
                db.Entry(forma_Pagamento).State = EntityState.Modified;
                db.SaveChanges();
                return Json(new { result = true, message = "Forma de Pagamento editada com sucesso" });
            } else {
                IEnumerable<ModelError> errors = ModelState.Values.SelectMany(i => i.Errors);
